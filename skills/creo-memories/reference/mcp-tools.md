...
すべてのID出力がEntId短縮形になっています。Base58エンコード + プレフィックスで型安全なIDを表現します。

| プレフィックス | エンティティ | 例 |
|--------------|-------------|-----|
| `mem_` | メモリ | `mem_2abc3def` |
| `usr_` | ユーザー | `usr_4ghi5jkl` |
| `ses_` | セッション（タスク） | `ses_6mno7pqr` |
| `mcp_` | MCPセッション | `mcp_8stu9vwx` |
| `atl_` | Atlas | `atl_9yzA1bcd` |
| `con_` | Concept | `con_Aabc1def` |
| `ctx_` | 共有コンテキスト | `ctx_Cmno3pqr` |
| `key_` | APIキー | `key_Dstu4vwx` |
| `team_` | チーム | `team_Eabc5def` |

**入力**: 以下の3形式すべてを受け付けます（`resolveToUuid` で自動変換）:
- EntId短縮形: `mem_2abc3def`
- UUID: `019ccc84-56b3-7f3e-8031-f6d32d0332a0`
- SurrealDB RecordId: `memories:019ccc84-...`
...
mcp__creo-memories__remember({
  ...
  slug: "my-memory",            // オプション（明示指定時のみ設定、B-2）
...
**Slug（addressing handle、B-2）**:
- 省略可。明示指定した場合のみ設定（auto採番なし）
- URL / `[[slug]]` wikilink での参照に使う
- charset: `^[a-z0-9][a-z0-9-]{1,62}$`
- 予約語 `personal` / `new` / `api` は使用不可
...
```typescript
mcp__creo-memories__concept_list({
...
```typescript
mcp__creo-memories__concept_create({
...
```typescript
mcp__creo-memories__concept_update({
  id: "con_xxx",                // 必須
  key: "new_key",               // オプション
  name: "新しい名前",           // オプション
  color: "#00FF00",             // オプション
...
```typescript
mcp__creo-memories__concept_delete({
  id: "con_xxx"                 // 必須
})
```
...
```typescript
mcp__creo-memories__concept_replace_for_memory({
  memory_id: "mem_xxx",         // 必須
...
mcp__creo-memories__create_atlas({
  name: "プロジェクトA",        // 必須
  description: "説明",          // オプション
  slug: "project-a",            // オプション（B-2、省略時は name から自動採番）
  display_name: "Project A ✨", // オプション（B-2、表示用ラベル。path には影響しない）
  parent_id: "atl_xxx",         // オプション（子Atlasの場合）
  metadata: {}                  // オプション
})
```

**Slug / display_name（B-2）**:
- `slug`: addressing handle。省略時は `name` から自動採番（衝突時は末尾に短い suffix）。指定時は charset `^[a-z0-9][a-z0-9-]{1,62}$` / 予約語（`personal`/`new`/`api`）不可 / global unique。set-once（後から変更不可）
- `display_name`: 表示用ラベル（自由文字列、emoji 可）。path には影響しない。省略時は `name` が表示される
...
mcp__creo-memories__get_atlas_tree({
  atlas_id: "atl_xxx"           // オプション（省略時は全ルートのフォレスト。id / slug / display-name のいずれでも指定可、B-2）
})
```
...
mcp__creo-memories__update_atlas({
  id: "atl_xxx",                // 必須（id / slug / display-name のいずれでも指定可、B-2）
  name: "新しい名前",           // オプション
  description: "新しい説明",    // オプション
  visibility: "public"          // オプション（public/private）
})
