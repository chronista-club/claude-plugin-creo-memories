...
現状 plugin は **72 tool** 提供。 4-scene 別に以下に網羅 (詳細 schema は `reference/mcp-tools.md`)。
...
> `create_atlas_shared_key` / `list_atlas_shared_keys` / `revoke_atlas_shared_key` / `delete_atlas_shared_key`（旧名 `*_domain_shared_key`）は現在マルチユーザー前提のため無効化中（creo-memories本体 #237、MCPには未公開）。

### 将来 (v1.0、 RFC v2)

72 tool → **6 core verbs + 5 named conveniences = 11 tools** にリデザイン進行中:
...
- [API Redesign Proposal](reference/api-redesign.md) — 72 tool → 11 tool 将来構想 (v0.23)
- [API Redesign RFC v1](reference/api-redesign-rfc.md) — formal spec + JSON schema + migration matrix + test scenarios (v0.28 NEW)
- [MCP Tools (現状)](reference/mcp-tools.md) — 72 tool 詳細
